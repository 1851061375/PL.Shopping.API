...
public class PersonalController : VersionNeutralApiController
{
    ...
    public async Task<ActionResult> UpdateProfileAsync(PersonalUpdateUserRequest request)
    {
    ...
    [HttpGet("tfa-setup")]
    [OpenApiOperation("Get audit logs of currently logged in user.", "")]
    public async Task<ActionResult<TfaSetupDto>> GetTfaSetupAsync(CancellationToken cancellationToken)
    {
        return User.GetUserId() is not { } userId || string.IsNullOrEmpty(userId)
        ? Unauthorized()
           : Ok(await _userService.GetTfaSetupAsync(userId, cancellationToken));
    }


    [HttpPost("tfa-setup")]
    [OpenApiOperation("Get audit logs of currently logged in user.", "")]
    public async Task<ActionResult<bool>> PostTfaSetupAsync(TfaSetupRequest request, CancellationToken cancellationToken)
    {
        return User.GetUserId() is not { } userId || string.IsNullOrEmpty(userId)
        ? Unauthorized()
           : Ok(await _userService.PostTfaSetupAsync(userId, request, cancellationToken));
    }
